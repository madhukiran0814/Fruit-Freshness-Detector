...
class AppTestCase(unittest.TestCase):
  ...
  def test_freshness_label(self):
    self.assertEqual(freshness_label(91), "Fresh")
    self.assertEqual(freshness_label(66), "Very Good")
    self.assertEqual(freshness_label(51), "Pretty Good")
    self.assertEqual(freshness_label(1), "Not Good")
    self.assertEqual(freshness_label(0), "Rotten")
  ...
  def test_get_index(self):
    ...
    self.assertTrue("Maturity Level" in resp_text)
    self.assertTrue("Price" in resp_text)
    self.assertTrue("Buy" in resp_text)
  ...
  def test_post_purchase(self):
    ...
    self.assertEqual(resp.status_code, 200)
    resp_text = resp.get_data(as_text=True)
    self.assertTrue("Thank You for Purchasing" in resp_text)
